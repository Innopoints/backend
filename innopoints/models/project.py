...
class Project(db.Model):
    ...
    lifetime_stage = db.Column(
        db.Enum(LifetimeStage), nullable=False, default=LifetimeStage.draft
    )

    files = db.relationship(
        "ProjectFile", cascade="all, delete-orphan", backref="project"
    )
    ...
    def start_date(self):
        ...
        return (
            db.session.query(db.func.min(Activity.start_date),)
            .filter(Activity.project_id == self.id,)
            .scalar()
        )
    ...
    def end_date(self):
        ...
        return (
            db.session.query(db.func.max(Activity.end_date),)
            .filter(Activity.project_id == self.id,)
            .scalar()
        )
