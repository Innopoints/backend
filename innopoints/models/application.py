...
class Application(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint(
            'applicant_email', 'activity_id', name='only one application'
        ),
    )
    ...
    activity_id = db.Column(
        db.Integer, db.ForeignKey('activities.id', ondelete='CASCADE'), nullable=False
    )
    comment = db.Column(db.String(1024), nullable=True)
    application_time = db.Column(
        db.DateTime(timezone=True), nullable=False, default=tz_aware_now
    )
    telegram_username = db.Column(db.String(32), nullable=True)
    status = db.Column(
        db.Enum(ApplicationStatus), nullable=False, default=ApplicationStatus.pending
    )
    actual_hours = db.Column(db.Integer, nullable=False)
    report = db.relationship(
        'VolunteeringReport', uselist=False, cascade='all, delete-orphan'
    )
    feedback = db.relationship('Feedback', uselist=False, cascade='all, delete-orphan')
...
class VolunteeringReport(db.Model):
    ...
    __table_args__ = (db.PrimaryKeyConstraint('application_id', 'reporter_email'),)
    ...
    rating = db.Column(
        db.Integer, db.CheckConstraint('rating <= 5 AND rating >= 1'), nullable=False
    )
...
class Feedback(db.Model):
    ...
    transaction = db.relationship(
        'Transaction', uselist=False, cascade='all, delete-orphan', passive_deletes=True
    )
