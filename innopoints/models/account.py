...
class Account(UserMixin, db.Model):
    ...
    created_projects = db.relationship(
        'Project', cascade='all, delete-orphan', backref='creator'
    )
    ...
    static_files = db.relationship(
        'StaticFile',
        cascade='all, delete-orphan',
        passive_deletes=True,
        backref='owner',
    )
    reports = db.relationship('VolunteeringReport', cascade='all, delete-orphan')
    ...
    def balance(self):
        ...
        return (
            db.session.query(db.func.sum(Transaction.change))
            .filter(Transaction.account_email == self.email)
            .scalar()
            or 0
        )
...
class Transaction(db.Model):
    ...
    stock_change_id = db.Column(
        db.Integer, db.ForeignKey('stock_changes.id'), nullable=True
    )
