...
class Activity(db.Model):
    ...
    __table_args__ = (
        db.CheckConstraint(
            f"(fixed_reward AND working_hours = 1) "
            f"OR (NOT fixed_reward AND reward_rate = {IPTS_PER_HOUR})",
            name="reward policy",
        ),
        db.UniqueConstraint(
            "name", "project_id", name="name is unique inside a project"
        ),
    )
    ...
    feedback_questions = db.Column(
        db.ARRAY(db.String(1024)), nullable=False, default=DEFAULT_QUESTIONS
    )
    ...
    def vacant_spots(self):
        ...
        accepted = Application.query.filter_by(
            activity_id=self.id, status=ApplicationStatus.approved
        ).count()
