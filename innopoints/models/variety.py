...
class Variety(db.Model):
    ...
    size = db.Column(
        db.String(3), db.ForeignKey("sizes.value", ondelete="CASCADE"), nullable=True
    )
    color = db.Column(
        db.String(6), db.ForeignKey("colors.value", ondelete="CASCADE"), nullable=True
    )
    images = db.relationship(
        "ProductImage", cascade="all, delete-orphan", passive_deletes=True
    )
    stock_changes = db.relationship(
        "StockChange", cascade="all, delete-orphan", passive_deletes=True
    )
    ...
    def amount(self):
        ...
        return (
            db.session.query(db.func.sum(StockChange.amount))
            .filter(
...
class ProductImage(db.Model):
    ...
    variety_id = db.Column(
        db.Integer, db.ForeignKey("varieties.id", ondelete="CASCADE"), nullable=False
    )
    image_id = db.Column(
        db.Integer, db.ForeignKey("static_files.id", ondelete="CASCADE"), nullable=False
    )
    order = db.Column(
        db.Integer,
...
class Color(db.Model):
    ...
    varieties = db.relationship(
        "Variety", cascade="all, delete-orphan", passive_deletes=True
    )
