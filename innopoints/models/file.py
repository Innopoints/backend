...
class StaticFile(db.Model):
    ...
    product_image = db.relationship(
        "ProductImage", uselist=False, cascade="all, delete-orphan"
    )
    project_file = db.relationship(
        "ProjectFile", uselist=False, cascade="all, delete-orphan"
    )
    cover_for = db.relationship("Project", uselist=False)
