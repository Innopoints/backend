...
def upgrade():
    ...
    op.create_unique_constraint('unique order indices', 'product_images', ['variety_id', 'order'], deferrable=True, initially='DEFERRED')
    op.create_unique_constraint('unique product', 'products', ['name', 'type'])
    op.create_unique_constraint('only one application', 'applications', ['applicant_email', 'activity_id'])
    ...
    op.create_check_constraint('not(feedback and stock_change)', 'transactions', '(stock_change_id IS NULL) OR (feedback_id IS NULL)')
...
def downgrade():
    ...
    op.create_check_constraint('feedback xor stock_change', 'transactions', '(stock_change_id IS NULL) != (feedback_id IS NULL)')
