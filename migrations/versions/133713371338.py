...
def downgrade():
    op.execute("ALTER TYPE notificationtype RENAME TO notificationtype_old;")
    op.execute(
        "CREATE TYPE notificationtype AS enum('purchase_ready', 'new_arrivals', 'claim_ipts', 'apl_accept', 'apl_reject', 'service', 'act_table_reject', 'all_feedback_in', 'out_of_stock', 'new_purchase', 'proj_final_review');"
    )
    op.execute(
        "ALTER TABLE notifications ALTER COLUMN type TYPE notificationtype USING type::text::notificationtype;"
    )
