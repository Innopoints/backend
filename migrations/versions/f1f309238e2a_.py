...
def upgrade():
    ...
    op.add_column('varieties', sa.Column('color_value', sa.String(length=6), nullable=True))
    op.create_primary_key(None, 'colors', ['value'])
    op.create_foreign_key(None, 'varieties', 'colors', ['color_value'], ['value'], ondelete='CASCADE')
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('varieties', sa.Column('color_id', sa.INTEGER(), autoincrement=False, nullable=True))
    op.drop_constraint(None, 'varieties', type_='foreignkey')
    op.create_foreign_key('varieties_color_id_fkey', 'varieties', 'colors', ['color_id'], ['id'], ondelete='CASCADE')
    op.drop_column('varieties', 'color_value')
    op.add_column('colors', sa.Column('id', sa.INTEGER(), server_default=sa.text("nextval('colors_id_seq'::regclass)"), autoincrement=True, nullable=False))
